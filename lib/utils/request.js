...
function doRequest (opts, limit) {
  ...
  const baseUrl = process.env.BASE_URL || 'https://play.google.com/store/apps';
  const gatewayProxy = process.env.GATEWAY_PROXY_URL || 'https://google-scrapers-api.azure-api.net';

  const url = opts.url.replace(baseUrl, gatewayProxy);
  const params = { ...opts, url };

  return new Promise((resolve, reject) => req(params, function (error, response, body) {
    if (error) {
      return reject(error);
    }
