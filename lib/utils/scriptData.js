...
*/
function extractor(mappings, fallbackMappings) {
  return function extractFields(parsedData) {
    debug('parsedData: %o', parsedData);

    const extractDataCallback = (spec) => {
      if (R.is(Array, spec)) {
    ...
    let result;
    try {
      result = R.map(extractDataCallback, mappings);
    } catch (err) {
      if(!fallbackMappings) {
        throw err;
      }

      result = R.map(extractDataCallback, fallbackMappings);
    }
    return result;
  };
}
...
function parseServiceRequests(response) {
  ...
  return eval(`(${valueMatch[0]})`);
}

function parseBatchResponse(response) {
  const input = JSON.parse(response.split('\n')[3]);
  const data = JSON.parse(R.path([0, 2], input));
  return data;
}

module.exports = { parse, parseServiceRequests, parseBatchResponse, extractor };
