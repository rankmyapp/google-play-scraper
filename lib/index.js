...
const cleanUrls = (req) => (_app) => Object.assign({}, _app, {
  playstoreUrl: _app.url,
  url: buildUrl(req, 'apps/' + _app.appId),
  permissions: buildUrl(req, 'apps/' + _app.appId + '/permissions'),
  similar: buildUrl(req, 'apps/' + _app.appId + '/similar'),
  reviews: buildUrl(req, 'apps/' + _app.appId + '/reviews'),
  developer: {
    devId: _app.developer,
    url: buildUrl(req, 'developers/' + qs.escape(_app.developer))
  }
});
...
router.post('/live-ops', async (req, res, next) => {
  const opts = req.body;
  gplay.liveOps(opts)
    .then((liveOps) => {
      res.json(liveOps);
