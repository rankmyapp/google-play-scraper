...
async function processAndRecur (html, opts, savedApps, mappings) {
  ...
  const appMapping = opts.clusterUrl ? mappings.cluster : mappings.apps;

  const processedApps = appList.extract(appMapping, html);
  const filteredApps = opts.num ? R.take(opts.num, processedApps) : processedApps;

  const apps = opts.fullDetail
    ? await processFullDetailApps(filteredApps, opts)
    : filteredApps;
